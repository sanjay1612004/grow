  ...
  return createPortal(
    ...
    >
      <div
        ...
        style={{ width: "419px", borderRadius: "12px", overflow: "hidden" }}
      ...
      >
        ...
        {/* Rows */}
        <div style={{ padding: "0 12px 12px 12px" }} className="flex flex-col gap-1">
          {watchlists.map((wl) => (
            <button
              key={wl.id}
              onClick={() => handleAdd(wl.id)}
              className="flex items-center w-full hover:bg-gray-50 transition-colors rounded-lg text-left"
              style={{ padding: "10px 8px", gap: "12px" }}
            >
              <div
              ...
              >
                <svg width="14" height="17" viewBox="0 0 14 17" fill="none">
                  <path
                    d="M1 1h12v14.5l-6-3-6 3V1z"
                    stroke="#00b386"
                    strokeWidth="1.4"
                    fill={added[wl.id] ? "#00b386" : "none"}
                  />
                </svg>
              </div>
              <span className="flex-1 text-gray-800 font-normal" style={{ fontSize: "14px" }}>
                {wl.name}
              </span>
              <span className="flex-shrink-0 font-normal" style={{ fontSize: "13px", color: "#9ca3af" }}>
                {wl.count} items
              </span>
            </button>
          ))}
          ...
          >
            ...
            <span className="font-medium" style={{ fontSize: "14px", color: "#00b386" }}>
...
// ── Stock Card ───────────────────────────────────────────────────────────────
function StockCard({ logo, name, price, change, changeVal, up, searchId, nse, bse, company }) {
  ...
  return (
    <>
      <div
        className="group relative flex flex-col cursor-pointer transition-colors duration-150 justify-between overflow-hidden border border-gray-200 hover:bg-[#f4f4f4] bg-white"
        style={{
          width: "188px",
          height: "182px",
          padding: "16px",
      ...
      >
        ...
        >
          <svg width="14" height="16" viewBox="0 0 14 16" fill="none">
            <path
              d="M1 1h12v14l-6-3-6 3V1z"
              stroke={bookmarked ? "#00b386" : "#9ca3af"}
              strokeWidth="1.3"
              fill={bookmarked ? "#00b386" : "none"}
            />
          </svg>
        </button>
...
// ── Stock Row ────────────────────────────────────────────────────────────────
function StockRow({ logo, name, price, change, changeVal, up, volume, searchId, nse, bse, company }) {
  const color = up ? "text-[#00b386]" : "text-[#eb5757]";
...
// ── News Card ────────────────────────────────────────────────────────────────
function NewsCard({ logo, name, change, up, headline, time, searchId, nse, bse, company }) {
  const color = up ? "text-[#00b386]" : "text-[#eb5757]";
  const navigate = useNavigate();
    ...
    >
      <div>
        ...
        <div className="border-l-2 border-gray-200 pl-3">
          <p className="text-sm text-gray-500 leading-relaxed text-left line-clamp-3">{headline}</p>
        </div>
      </div>
...
function SectorRow({ icon, name, gainers, losers, change }) {
  ...
  return (
    <div className="flex items-center px-4 py-3 border-b border-gray-50 hover:bg-gray-50 cursor-pointer">
      ...
      <div className={`text-sm font-medium w-20 text-right ${up ? "text-[#00b386]" : "text-[#eb5757]"}`}>
        {change}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3.5 border-b border-gray-200 hover:bg-gray-50 cursor-pointer transition-colors">
      <div className="flex flex-col items-start min-w-0 flex-1">
        <div className="text-sm font-medium text-gray-900 mb-1.5 truncate w-full">{title}</div>
        <span className={`text-[11px] font-semibold px-2 py-0.5 rounded ${bg}`}>{label}</span>
      </div>
  ...
  useEffect(() => {
    const fetchPopularStocksMostBought = async () => {
      try {
        ...
        setmtfStocks(rawStocks.map((item) => {
        ...
        console.error("Failed to fetch Popular Stocks Most Bought:", error);
      }
    };
  ...
  useEffect(() => {
    const fetchMostBoughtStocks = async () => {
      try {
        ...
        setMostBought(rawStocks.map((item) => {
          const ltp = item.stats.ltp || 0;
          const dayChange = item.stats.dayChange || 0;
          const dayChangePerc = item.stats.dayChangePerc || 0;
          const isUp = dayChange >= 0;
  ...
  useEffect(() => {
    const fetchTopintradaystocks = async () => {
      try {
        ...
        setTopintradaystocks(rawStocks.map((item) => {
          const ltp = item.stats.ltp || 0;
          const dayChange = item.stats.dayChange || 0;
          const dayChangePerc = item.stats.dayChangePerc || 0;
          const isUp = dayChange >= 0;
        ...
        console.error("Failed to fetch top intraday stocks:", error);
      }
    };
  ...
  useEffect(() => {
    const fetchETFstocks = async () => {
      try {
        ...
        setMostboughtETFs(rawStocks.map((item) => {
          const ltp = item?.stats?.ltp || 0;
          const dayChange = item?.stats?.dayChange || 0;
          const dayChangePerc = item?.stats?.dayChangePerc || 0;
          const isUp = dayChange >= 0;
  ...
  useEffect(() => {
    const fetchStocksInNewsToday = async () => {
      try {
        ...
        setStocksinnewstoday(rawNews.map((item) => {
          ...
          return {
            ...
            company: cta?.ctaText || "",
            searchId: cta?.ctaUrl ? cta.ctaUrl.split("/stocks/")[1] : "",
          };
        }));
      } catch (error) {
  ...
  useEffect(() => {
    const fetchTrendingSectors = async () => {
      try {
        ...
        setSectorstrendingtoday(rawSectors.map((item) => ({
  ...
  useEffect(() => {
    const fetchETFsbyGrowwstocks = async () => {
      try {
        ...
        setETFsbyGroww(rawStocks.map((item) => {
          const ltp = item?.stats?.ltp ?? 0;
          const dayChange = item?.stats?.dayChange ?? 0;
          const dayChangePerc = item?.stats?.dayChangePerc ?? 0;
          const isUp = dayChange >= 0;
      ...
      <main className="max-w-[1400px] mx-auto px-1 py-1">
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0 space-y-8">
            ...
            {/* Most bought stocks on Groww */}
            <section>
              <h2 className="text-base font-semibold text-gray-900 mb-4">Most bought stocks on Groww</h2>
              <div className="grid grid-cols-4 gap-3">
                {mtfStocks.map((s) => <StockCard key={s.searchId} {...s} />)}
              </div>
              <a className="mt-4 text-sm text-[#00b386] font-medium hover:underline flex items-center gap-1" href="/stocks/most-bought-stocks-on-groww">
                See more <svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg>
              </a>
            </section>
            ...
            <section>
              ...
              <div className="bg-white border border-gray-300 rounded-xl overflow-hidden">
                ...
                {moversLoading ? (
                  ...
                  <div className="flex items-center justify-center py-10 text-sm text-gray-400">No data available</div>
                ) : (
                  topMovers.map((s) => <StockRow key={s.searchId} {...s} />)
                )}
              </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-4 gap-3">
                {Topintradaystocks.map((s) => <StockCard key={s.searchId} {...s} />)}
              </div>
            ...
            <section>
              ...
              <div className="bg-white border border-gray-300 rounded-lg overflow-hidden">
                <div className="flex items-center px-4 py-2 bg-gray-50 border-b border-gray-100">
                  ...
                  <div className="text-xs text-gray-500" style={{ marginRight: "6rem" }}>Gainers/Losers</div>
            ...
            <section>
              ...
              <div className="grid grid-cols-4 gap-3">
                {MostboughtETFs.map((s) => <StockCard key={s.searchId} {...s} />)}
              </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-4 gap-3">
                {ETFsbyGroww.map((s) => <StockCard key={s.searchId + "-groww"} {...s} />)}
              </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-4">
                {Stocksinnewstoday.map((s) => <NewsCard key={s.searchId} {...s} />)}
              </div>
              <a className="mt-4 text-sm text-[#00b386] font-medium hover:underline flex items-center gap-1" href="/market-news/stocks">
                See more news <svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg>
              </a>
            </section>
          ...
          <aside className="w-[360px] flex-shrink-0 space-y-4 h-fit self-start">
            ...
            <div className="bg-white border border-gray-300 rounded-xl overflow-hidden">
              {products.map((p, i) => (
                <div
                  key={p.name}
                  className={`flex items-center justify-between px-4 py-3 hover:bg-gray-50 cursor-pointer ${i < products.length - 1 ? "border-b border-gray-50" : ""}`}
                >
            ...
            <div className="sticky top-25 mt-30">
              ...
              <div className="bg-white border border-gray-300 rounded-xl overflow-hidden shadow-sm">
                {tradingScreens.map((t) => <TradingCard key={t.title} {...t} />)}
