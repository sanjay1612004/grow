...
function SkeletonRow() {
  return (
    <div className="grid grid-cols-[1.8fr_140px_160px_90px_110px_110px] items-center border-b border-gray-100 animate-pulse">
...
function InvestCard({ selectedStock }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-4 flex flex-col items-center text-center gap-4 sticky top-6">
...
function StockRow({ item, isLast, onClick, isSelected }) {
  ...
  return (
    <div
      ...
      className={`grid grid-cols-[1.8fr_160px_140px_90px_110px_110px] items-center transition-colors duration-100 hover:bg-gray-50 cursor-pointer ${
    ...
    >
      ...
      <div className="flex items-center gap-3 px-5 py-4 min-w-0">
        ...
        {/* Fallback */}
        <div className="w-20 h-10 rounded-lg bg-gray-100 text-gray-500 text-[10px] font-bold items-center justify-center flex-shrink-0 hidden">
          {company.companyShortName?.slice(0, 2).toUpperCase()}
        </div>
        <span
          className="text-[12.5px] font-medium text-gray-800 hover:text-[#00b386] hover:underline"
          onClick={(e) => {
            e.stopPropagation();
...
export default function GrowwMostTraded() {
    ...
    const fetchStocks = async () => {
      ...
      try {
        const res = await fetch(
          "https://groww.in/v1/api/stocks_data/v2/explore/list/top?discoveryFilterTypes=POPULAR_STOCKS_MOST_BOUGHT_MTF&page=0&size=20",
          { headers: { Accept: "application/json" } }
        );
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        if (!cancelled) {
          const fetchedStocks = data?.exploreCompanies?.POPULAR_STOCKS_MOST_BOUGHT_MTF ?? [];
  ...
  return (
    <div>
      ...
      <div className="min-h-screen bg-white font-sans">
        <div className="max-w-6xl mx-12 px-6 py-8">
          ...
          <div className="flex gap-6 items-start my-7">
            ...
            <div className="flex-1 bg-white rounded-2xl border border-gray-200 overflow-hidden">
              ...
              {/* Table header */}
              <div className="grid grid-cols-[1.8fr_140px_160px_90px_110px_110px] border-b border-gray-100">
                <div className="px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">
                  Company
                </div>
                <div className="px-4 py-3" />
                <div className="px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide text-right">
                  Market Price (1D)
                </div>
                <div className="px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide text-right">
                  Haircut
                ...
                <div className="px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide text-right">
                  1 Day High
                </div>
              </div>
